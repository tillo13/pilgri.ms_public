"""
Bug #1405 — apply the depot cost rebalance computed by tools/analyze_depot_costs.py.

Same algorithm as the analyzer (locked + greenlit by Luke 2026-05-06):
  - L1-L3: unchanged
  - L4 floor = round(MID_TARGET_HRS × Andy's shards/hr), then ×1.12 ramp through L7
  - L8 floor = round(LATE_TARGET_HRS × Luke's shards/hr), then ×1.12 ramp through L10
  - max(existing_cost, ramped_floor) — never lower an already-high cost
  - Anchors are FROZEN to the values from analyzer comment #200 (Andy=103.2/hr, Luke=546.4/hr)
    so the patch is reproducible regardless of live shards/hr drift.
...
LATE_L8_FLOOR = int(round(LATE_TARGET_HRS * ANCHOR_LUKE_RATE))    # 54640


def proposed_cost(level: int, current_cost: int) -> int:
...
def main():
    ...
    print(f'Anchors: MID L4 floor = {MID_L4_FLOOR} (40h × {ANCHOR_ANDY_RATE}/hr), '
          f'LATE L8 floor = {LATE_L8_FLOOR} (100h × {ANCHOR_LUKE_RATE}/hr), ramp = {RAMP}x/lvl')
    print()
